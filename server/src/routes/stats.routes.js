...
router.get('/', async (req, res) => {
  try {
    ...
    const [snap, agentsSnap, areasSnap, urgentSnap, awaitingSnap] = await Promise.all([
      db.collection('bot-base_conversations')
        .where('updatedAt', '>=', startTs)
        .where('updatedAt', '<=', endTs)
        .get(),
      db.collection('bot-base_agents').get(),
      db.collection('bot-base_areas').get(),
      db.collection('bot-base_conversations').where('urgent', '==', true).get(),
      db.collection('bot-base_conversations').where('status', '==', 'escalated').get(),
    ]);
